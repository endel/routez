...
1. **Compression** (S–M). Serve precompressed `.gz`/`.br` files; Brotli on
   the fly.
2. **Static file I/O off the worker thread** (M). A slow disk stalls the
   worker today.
3. **Client certificates to upstreams** (S). quic-zig's `tls_client` can
   present one; routez needs the config and key loading.

## Later: larger projects, or waiting on a need

4. **Zero-downtime QUIC reloads** (L). New connections that land on the old
   generation's sockets are refused for up to 10 s; needs socket handover or
   eBPF steering.
5. **Response caching** like nginx's `proxy_cache` (L).
6. **WebSockets over HTTP/3** (RFC 9220) (M). Browsers don't use it yet;
   worth doing once they do, for real-time traffic.
7. **HTTP/2** (L), with WebSockets over it (RFC 8441). Needed for gRPC and
...
8. **ACME DNS-01** for wildcard certificates, and OCSP stapling (M).
9. **Layer-4 TCP proxy** beside the UDP one (S).
10. **quic-zig idle timeout covering PTO backoff.** A patch exists, parked:
