...
Nothing queued; the list below is next, in order, as needs arrive.

## Later: larger projects, or waiting on a need

1. **Zero-downtime QUIC reloads** (L). New connections that land on the old
   generation's sockets are refused for up to 10 s; needs socket handover or
   eBPF steering.
2. **Response caching** like nginx's `proxy_cache` (L).
3. **WebSockets over HTTP/3** (RFC 9220) (M). Browsers don't use it yet;
   worth doing once they do, for real-time traffic.
4. **HTTP/2** (L), with WebSockets over it (RFC 8441). Needed for gRPC and
