...
pub fn main(init: std.process.Init) !u8 {
    ...
    const ca = args[2];
    // quic-zig's echo server answers each chunk as "Echo: <chunk>" from a 1 KB buffer.
    const payload = try init.arena.allocator().alloc(u8, 500);
    for (payload, 0..) |*b, i| b.* = 'a' + @as(u8, @intCast(i % 26));

    var handler: Client = .{ .payload = payload };
    ...
    const echoed = std.mem.startsWith(u8, handler.stream_echo.items, "Echo: ") and std.mem.endsWith(u8, handler.stream_echo.items, payload);
